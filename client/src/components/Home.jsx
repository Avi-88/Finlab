...
const getCurrentUser = async()=>{
  try {
    const res = await axios(`http://localhost:8000/api/users/${user._id}`);
  ...
  const handleAddCoin = async(id) =>{
    if(isLoggedIn){
    try {
      const res = await axios.put(`http://localhost:8000/api/users/${id}/addcoin`, {userId: user._id});
      if(res.status === 200){
      setOpenAlert(true);
  ...
  const handleRemoveCoin = async(id) =>{
    if(isLoggedIn){
    try {
      const res = await axios.put(`http://localhost:8000/api/users/${id}/removecoin`, {userId: user._id});
      if(res.status === 200){
        setOpenAlert(true);
